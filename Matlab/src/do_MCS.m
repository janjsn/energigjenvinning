...
for i = 1:c-1
    ...
    for j = 1:n_simulations
        ...
        residual_share = 1-(MCS_in.msw_share_plastic_mass(j)+MCS_in.msw_share_inert_mass(j) + MCS_in.msw_share_paper_mass(j) + MCS_in.msw_share_biowaste_mass(j));
        ...
        oxidized_biogenic_CH4_to_CO2_kgCO2 = landfill_gas_biogenic_this_kgCH4*(1-methane_collection_rate)*(MCS_in.landfill_methane_oxidation_factor(j))*(MCS_in.molar_mass_CO2(j)/MCS_in.molar_mass_CH4(j));
        ...
        if MCS_in.binary_avoided_Norwegian_heat_is_electricity_boiler == 1
            MCS_out(i).avoided_energy_production_Norway_kgCO2eq(j) =  MCS_out(i).incineration_overcapacity*10^-3*MCS_out(i).energy_produced_heat_kWh(j)*MCS_in.emission_intensity_norwegian_electricity_gCO2eq_per_kWh(j);
        else
            MCS_out(i).avoided_energy_production_Norway_kgCO2eq(j) = MCS_out(i).incineration_overcapacity*10^-3*MCS_out(i).energy_produced_heat_kWh(j)*(MCS_in.lca_emissions_non_energy_heat_pump_gCO2eq_per_kWh(j)+...
